...
struct inode* create_inode(struct superblock *sb)
{
    ...
    spin_lock_init(&new_inode_ret->i_lock);

    lru_node_reset(&new_inode_ret->i_lru_cache_node);
...
static struct lru_ops inode_lru_ops = {
    .free = icache_lru_free,
    .sync = icache_lru_sync,
};

static struct hash_ops inode_hash_ops = {
    .hash_func = icache_lru_hash,
    .hash_compare = icache_lru_compare,
};

int icache_init()
{
    global_inode_cache = lru_cache_create(128, &inode_lru_ops, &inode_hash_ops);
...
struct inode *iget(struct superblock *sb, ino_t ino) 
{
    ...
    // 先从缓存中查找
    struct lru_node *found_node = lru_cache_find(global_inode_cache, &temp_inode.i_lru_cache_node);
    if (found_node) 
    {
        // 找到，返回缓存中的inode
    ...
    if(sb->s_ops->read_inode(new_inode_ret)>=0)
    {
        ...
        lru_cache_add(global_inode_cache, &new_inode_ret->i_lru_cache_node);
        inode_unlock(new_inode_ret);
        return new_inode_ret;
    }
...
int iput(struct inode *inode) 
{
    ...
    if (inode->i_refcount > 0)
    {
        ...
        if (inode->i_refcount > 0)
        {
            lru_cache_touch(global_inode_cache, &inode->i_lru_cache_node);
            inode_unlock(inode);
            return 0;
        }
    }
    inode_unlock(inode);
    lru_cache_remove(global_inode_cache, &inode->i_lru_cache_node);
    return 0;
}
...
int icache_sync_all()
{
    ...
    lru_cache_walk(global_inode_cache, icache_lru_sync);
    return 0;
}
