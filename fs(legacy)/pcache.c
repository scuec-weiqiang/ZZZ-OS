...
static struct page_cache *create_page(struct inode *inode, pgoff_t index)
{
    ...
    lru_node_reset(&p->p_lru_cache_node);
...
static struct lru_ops page_lru_ops = {
    .free = vfs_global_page_lru_free,
    .sync = vfs_pcache_sync_func,
};

static struct hash_ops page_hash_ops = {
    .hash_func = vfs_global_page_lru_hash,
    .hash_compare = vfs_global_page_lru_compare,
};

int pcache_init()
{
    global_page_cache = lru_cache_create(128, &page_lru_ops, &page_hash_ops);
...
struct page_cache *pget(struct inode *inode, u32 index)
{
    ...
    lru_cache_add(global_page_cache, &found_page->p_lru_cache_node);
    page_unlock(found_page);
...
int pput(struct page_cache *page)
{
    ...
    if (page->refcount > 0)
    {
        ...
        if (page->refcount > 0)
        {
            lru_cache_touch(global_page_cache, &page->p_lru_cache_node);
            page_unlock(page);
            return 0;
        }
    }
    page_unlock(page);
    lru_cache_remove(global_page_cache, &page->p_lru_cache_node);
    return 0;
}
...
int pcache_sync_all()
{
    ...
    lru_cache_walk(global_page_cache, vfs_pcache_sync_func);
    return 0;
}
