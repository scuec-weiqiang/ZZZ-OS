...
static u64 pages_num = 0;

u64 remain_mem = RAM_SIZE;

void print_maddr()
{
    printk("_text_start = %x---->",text_start);printk("_text_end = %x\n",text_end);
    printk("_rodata_start = %x---->",rodata_start);printk("_rodata_end = %x\n",rodata_end);
    printk("_data_start = %x---->",data_start);printk("_data_end = %x\n",data_end);
    printk("_bss_start = %x---->",bss_start);printk("_bss_end = %x\n",bss_end);
    printk("_heap_start = %x---->",heap_start);printk("_heap_end = %x\n",heap_end);
    printk("_heap_size = %x\n",heap_size);
    printk("_stack_start = %x---->",stack_start);printk("_stack_end = %x\n",stack_end);
...
{   
    ...
    for(;(u64)pagem_i < reserved_end; pagem_i++)
    {
        ...
        else//找到了，对pagem_i到pagem_j标志位置1，表明他们管理的内存被占用了
        {
            ...
            uintptr_t pgaddr =  alloc_start + ((((uintptr_t)pagem_i - (uintptr_t)heap_start)/sizeof(PageM_t))*PAGE_SIZE);
            // printk("pgaddr = %x\n",pgaddr);
...
void print_page(u64 start,u64 end)
{
    ...
    {
        printk("pagem %x ->>%x = %x\n",pagem_i, alloc_start + ((((uintptr_t)pagem_i - (uintptr_t)heap_start)/sizeof(PageM_t))*PAGE_SIZE),_IS_FREE(pagem_i));
        pagem_i++;
    }
}
