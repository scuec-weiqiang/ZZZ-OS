...
struct hashtable* hashtable_init(size_t num_buckets, struct hash_ops *ops) {
    ...
    ASSERT(ops != NULL, "Hash operations must not be NULL");
    ASSERT(ops->hash_func != NULL, "Hash function must not be NULL");
    ASSERT(ops->hash_compare != NULL, "Key compare function must not be NULL");
    ...
    if (!ht->buckets) {
        printk("%s\n", "Memory allocation for buckets failed");
        return NULL;
    }
